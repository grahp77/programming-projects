...
void mul_by_10(s21_big_decimal *dst) {
  ...
  dst->bits[7] =
      (dst->bits[7] & ~(255 << 16)) | ((big_degree(temp1) + 1) << 16);
}
...
void SAR(s21_big_decimal *reduced, s21_big_decimal divisible,
         s21_big_decimal deductible, int *bit_pos, s21_big_decimal *result) {
  int flag = 0;
...
int logic(s21_big_decimal value_1, s21_big_decimal value_2,
          s21_decimal result) {
  int flag = 0;
...
void add(s21_big_decimal temp1, s21_big_decimal temp2,
...
void sub(s21_big_decimal temp1, s21_big_decimal temp2,
         s21_big_decimal *result) {
  big_zero(result);
  int prev_ch = 0;
...
void rounding_up(s21_big_decimal *dst, int *deg) {
  ...
  if ((z % 20 > 5 && z % 20 < 10) || (z % 20 >= 15 && z % 20 <= 19) ||
      (z % 20 == 5 && remains)) {
    add(*dst, one, dst);
  }
}

void round_decimal(s21_big_decimal *dst, int *deg) {
  int z = 0;
  while (used_bits(*dst) > 96 || *deg > 28) {
    z = last_number(*dst, 2);
    div_by_10(dst);
...
int s21_supprot_division(s21_big_decimal temp_res, int deg, int sign) {
  s21_big_decimal temp_res_big = {{0, 0, 0, 1, 0, 0, 0, 0}};
  s21_big_decimal temp_res_NULL = {{0, 0, 0, 0, 0, 0, 0, 0}};
  int error = 0;

  if ((deg < 0 && sign) ||
      (unsigned_comparison(temp_res, temp_res_NULL) == 26 && sign) ||
      (unsigned_comparison(temp_res_big, temp_res) == 26 && sign) || deg > 28) {
    error = 2;
  } else if ((deg < 0 && !sign) ||
             (unsigned_comparison(temp_res_big, temp_res) == 26 && !sign) ||
             (unsigned_comparison(temp_res, temp_res_NULL) == 26 && !sign)) {
    error = 1;
  }
  return error;
